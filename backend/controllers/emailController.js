...
// Send a single email
const sendEmail = async (recipient, template, userId, transporter) => {
    try {
        ...
        for (const email of recipients) {
            try {
                ...
                await transporter.sendMail({
                    ...
                    subject: subject,
                    text: body,
                    html: body // Assuming HTML is supported
                });
...
exports.getEmailAnalytics = async (req, res) => {
    try {
        ...
        const sentCount = await EmailLog.countDocuments({ ...query, status: 'sent' });
        const failedCount = await EmailLog.countDocuments({ ...query, status: 'failed' });
        const repliedCount = await EmailLog.countDocuments({ ...query, status: 'replied' });
        const interviewedCount = await EmailLog.countDocuments({ ...query, status: 'interviewed' });
        ...
        res.status(200).json({
            analytics: {
                ...
                interviewRate: sentCount > 0 ? (interviewedCount / sentCount) * 100 : 0
            }
        });
    } catch (error) {
